...
def citas_guardar(request):
    if request.method == "POST":
            ...
            try:
                ...
                if cliente.rol == 3:
                    try:
                        ...
                        fecha_hora_datetime = datetime.strptime(fecha_hora, '%Y-%m-%dT%H:%M')

                        mensaje = f"""
                            <h1 style='color:blue;'>¡Cita agendada exitosamente!</h1>
                            <p>Datos de su cita:</p>
                            <p>-Fecha y hora: {fecha_hora_datetime}</p>
                            <p>-Servicio: {servicio}</p>
                            <p>-Precio: ${precio}</p>
                            <h3 style='color:green;'>¡Gracias por preferir +Cotitas!</h3>
                        """

                        try:
                            msg = EmailMessage("+Cotitas", mensaje, settings.EMAIL_HOST_USER, [destinatario])
                            msg.content_subtype = "html"  # Habilitar html
                            msg.send()
                        except BadHeaderError:
                            return HttpResponse("Invalid header found.")
        ...
        if cliente.rol == 3:
            return HttpResponseRedirect(reverse("tienda:mis_citas"))
        else:
            return HttpResponseRedirect(reverse("tienda:citas"))
...
def mis_citas(request):
    usuario = request.session.get("logueo", False)
    citas = Cita.objects.filter(cliente=usuario["id"])
    contexto = {'data': citas}
    return render(request, "tienda/usuarios/mis_citas.html", contexto)
